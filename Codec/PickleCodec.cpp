...
namespace {
    ...
    struct StackItem {
        StackItem() : Type(0), Value() {}

        explicit StackItem(int type, ojson const &value = ojson()) : Type(type), Value(value) {
            //
        }
...
void PickleCodec::Encode(std::ostream & /*stream*/, ojson const & /*root*/) const { throw NotImplementedException(__func__); }
