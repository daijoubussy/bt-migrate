...
ImportHelper::Result::Result() : SuccessCount(0), FailCount(0), SkipCount(0) {
    //
}

ImportHelper::ImportHelper(ITorrentStateStorePtr sourceStore, fs::path const &sourceDataDir, ITorrentStateStorePtr targetStore, fs::path const &targetDataDir,
...
ImportHelper::Result ImportHelper::Import(unsigned int threadCount) {
    ...
    try {
        ...
        for (unsigned int i = 0; i < threadCount; ++i) {
            threads.emplace_back(&ImportHelper::ImportImpl, this, std::cref(m_targetDataDir), std::ref(*boxes), std::ref(result));
        }
    ...
    Logger(Logger::Info) << "Finished: " << result.SuccessCount << " succeeded, " << result.FailCount << " failed, " << result.SkipCount << " skipped";
