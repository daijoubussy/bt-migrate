...
namespace {
    namespace Detail {
        ...
        enum Priority { DoNotDownloadPriority = 0, MinPriority = -6, MaxPriority = 6 };
        ...
        fs::path GetStateDir(fs::path const &dataDir) { return dataDir / "state"; }
...
namespace {
    ...
    Box::LimitInfo FromStoreSpeedLimit(ojson const &storeLimit) {
        Box::LimitInfo result;
        result.Mode = storeLimit.as<int>() > 0 ? Box::LimitMode::Enabled : (storeLimit.as<int>() == 0 ? Box::LimitMode::Disabled : Box::LimitMode::Inherit);
    ...
    class DelugeTorrentStateIterator : public ITorrentStateIterator {
       public:
        DelugeTorrentStateIterator(fs::path const &stateDir, ojson &&fastResume, ojson &&state, IFileStreamProvider const &fileStreamProvider);
    ...
    DelugeTorrentStateIterator::DelugeTorrentStateIterator(fs::path const &stateDir, ojson &&fastResume, ojson &&state, IFileStreamProvider const &fileStreamProvider)
        : m_stateDir(stateDir),
    ...
    bool DelugeTorrentStateIterator::GetNext(Box &nextBox) {
        ...
        box.CorruptedSize = 0;
        box.SavePath =
            Util::GetPath(state[STField::SavePath].as<std::string>()) /
            (fastResume.contains(FRField::MappedFiles) ? *Util::GetPath(fastResume[FRField::MappedFiles][0].as<std::string>()).begin() : Util::GetPath(box.Torrent.GetName()));
        ...
        for (std::size_t i = 0; i < filePriorities.size(); ++i) {
            ...
            file.Priority = file.DoNotDownload ? Box::NormalPriority : BoxHelper::Priority::FromStore(filePriority - 1, Detail::MinPriority, Detail::MaxPriority);
    ...
    bool DelugeTorrentStateIterator::GetNext(fs::path &torrentFilePath, ojson &state, std::string &fastResumeData) {
        ...
        for (; m_stateIt != m_stateEnd; ++m_stateIt) {
            ...
            if (torrentIdIt == state.object_range().end()) {
                Logger(Logger::Warning) << "Torrent ID is missing from state entry #" << std::distance(m_state[Detail::StateField::Torrents].array_range().begin(), m_stateIt)
                                        << ", skipping";
                continue;
            }
...
TorrentClient::Enum DelugeStateStore::GetTorrentClient() const { return TorrentClient::Deluge; }
...
bool DelugeStateStore::IsValidDataDir(fs::path const &dataDir, Intention::Enum /*intention*/) const {
    ...
    return fs::is_regular_file(stateDir / Detail::FastResumeFilename) && fs::is_regular_file(stateDir / Detail::StateFilename);
}
...
void DelugeStateStore::Import(fs::path const & /*dataDir*/, Box const & /*box*/, IFileStreamProvider & /*fileStreamProvider*/) const { throw NotImplementedException(__func__); }
