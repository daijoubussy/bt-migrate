...
namespace {
    namespace Detail {
        ...
        struct ResumeInfo {
            ...
            ResumeInfo Copy() const { return {InfoHash, ResumeData, SavePath != nullptr ? std::make_unique<std::string>(*SavePath) : nullptr}; }
        };
...
namespace {
    ...
    auto OpenResumeDatabase(fs::path const &path) {
        ...
        return make_storage(path.string(), make_table("TORRENTS", make_column("INFOHASH", &Detail::ResumeInfo::InfoHash, primary_key()),
                                                      make_column("RESUME", &Detail::ResumeInfo::ResumeData), make_column("SAVE_PATH", &Detail::ResumeInfo::SavePath)));
    }
    ...
    bool uTorrentWebTorrentStateIterator::GetNext(Box &nextBox) {
        ...
        box.Torrent = ojson{ojson::object{{TField::Info, resume[RField::Info]}, {TField::UrlList, resume.get_with_default(RField::UrlList, ojson{ojson::array{}})}}};
...
TorrentClient::Enum uTorrentWebStateStore::GetTorrentClient() const { return TorrentClient::uTorrentWeb; }
...
bool uTorrentWebStateStore::IsValidDataDir(fs::path const &dataDir, Intention::Enum /*intention*/) const {
    return fs::is_regular_file(dataDir / Detail::ResumeFilename) && fs::is_regular_file(dataDir / Detail::StoreFilename);
}
...
void uTorrentWebStateStore::Import(fs::path const & /*dataDir*/, Box const & /*box*/, IFileStreamProvider & /*fileStreamProvider*/) const {
